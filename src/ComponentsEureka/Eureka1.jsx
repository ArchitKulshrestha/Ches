      ...
      <section className="bg-background-950">
        ...
        <div className="flex flex-col justify-center items-center ">
          {EurekaA.map((eurekaA, index) => (
            <div
              className="bg-background-900 flex w-[83%] flex-col justify-center items-center py-4 mb-4 rounded-lg hover:shadow-lg hover:shadow-text-500/60 backdrop-blur-[10px]"
              key={index}>
              <img src={eurekaA.img} alt="" className="w-[80%]" />
              <p className="text-text-100 text-xl px-1 text-center font-semibold mt-1">
                {eurekaA.title}
              </p>
              <p className="text-text-100 text-center font-[500] mt-1">
                {eurekaA.author}
              </p>
            </div>
          ))}
